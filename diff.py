...
parser = argparse.ArgumentParser(description="Diff PowerPC assembly.")
...
for binutils_cand in ["powerpc-linux-gnu-", "powerpc64-elf-"]:
    try:
...
if not binutils_prefix:
    fail(
        "Missing binutils; please ensure powerpc-linux-gnu-objdump or powerpc64-elf-objdump exist."
    )
...
def dump_binary():
    ...
    objdump_flags = ["-Dz", "-bbinary", "-EB", "-mpowerpc", "-M750cl"]
    flags1 = [
...
re_regs = re.compile(r"r[0-3]?[0-9]|f[0-3]?[0-9]|lr|spr[0-2]?[0-9]?[0-9]\b")
re_sprel = re.compile(r",([0-9]+|0x[0-9a-f]+)\(sp\)")
re_large_imm = re.compile(r"-?[1-9][0-9]{2,}|-?0x[0-9a-f]{3,}")
re_imm = re.compile(r"(\b|-)([0-9]+|0x[0-9a-fA-F]+)\b(?!\(sp)|%(lo|hi)\([^)]*\)")
forbidden = set(string.ascii_letters + "_")
branch_instructions = re.compile(r"\bb(c|eq|ge|gt|le|lt|ne|ng|nl|ns|so)?l?a?\b")
...
def process(lines):
    ...
    for row in lines:
        ...
        row_parts = re.sub(r"\s+", "\t", row).split("\t", 1)
        ...
        if re.search(branch_instructions, mnemonic):
            target = row_parts[1].strip()[-1]
