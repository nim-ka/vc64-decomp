...
def create_project_settings(settings: Dict[str, Any]) -> ProjectSettings:
    return ProjectSettings(
        ...
        objdump_executable=os.path.expandvars(get_objdump_executable(settings.get("objdump_executable"))),
...
def run_objdump(cmd: ObjdumpCommand, config: Config, project: ProjectSettings) -> str:
    ...
    try:
        out = subprocess.run(
            [project.objdump_executable] + config.arch.arch_flags + flags + ["-mpowerpc", "-M750cl"] + [target],
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
